...
- **Reddit**: anonymous `/r/{subreddit}/new.json` requests are blocked with HTTP 403 and HTML responses from the current runner environment. OAuth is required. Reddit's free OAuth tier allows 100 queries per minute for non-commercial use. Until OAuth lands, the source registry marks only HTTP 401/403 as an expected failure; a timeout, DNS error, or server error remains a real failure, and an unexpected success is called out so the flag can be removed. The intended communities remain `r/coolgithubprojects`, `r/SideProject`, `r/opensource`, `r/webdev`, `r/programming`, `r/selfhosted`, `r/homelab`, and `r/raspberry_pi`.
